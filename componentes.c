...
void inicializarComponentes() {
    ...
    if (componentes == NULL) {
        printf("Erro ao alocar memória para componentes.\n");
        exit(1);
    }
...
void listarComponentes() {
    ...
    for (int i = 0; i < totalComponentes; i++) {
        printf("ID: %d | Designação: %s | Nº Série: %s | Data Aquisição: %s | Garantia: %d meses | Tipo: %s | Condição: %s\n", 
                componentes[i].id, 
                componentes[i].designacao,
                componentes[i].numeroSerie, 
                componentes[i].dataAquisicao,
...
void pesquisarComponente() {
    ...
    for (int i = 0; i < totalComponentes; i++) {
        if (componentes[i].id == id) {
            printf("ID: %d | Designação: %s | Nº Série: %s | Data Aquisição: %s | Garantia: %d meses | Tipo: %s | Condição: %s | Observações: %s\n", 
                    componentes[i].id, 
                    componentes[i].designacao,
                    componentes[i].numeroSerie, 
                    componentes[i].dataAquisicao,
...
void alterarComponente() {
    ...
    for (int i = 0; i < totalComponentes; i++) {
        if (componentes[i].id == id) {
            ...
            printf("Novo tipo: ");
            scanf(" %[^\n]", componentes[i].tipo);
            printf("Nova condição (Novo/Usado): ");
            scanf(" %[^\n]", componentes[i].condicao);
